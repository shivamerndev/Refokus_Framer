...
const Navbar = () => {
  return (
    <div className='border-b-[1px] border-zinc-600 font-semibold text-2xl max-w-screen-xl mx-auto px-8 py-6 flex items-center justify-between font-[Plain] '>
      <div className='h-4 w-fit flex items-center gap-2 '>
        ...
        <p className='text-sm font-semibold'>Refokus</p>
      </div>
      <div className='flex gap-12 justify-start mr-80'>
        {['Home', 'Work', 'Culture', '', 'News'].map((e, i) => (
          <a key={i} className='text-sm font-light flex items-center gap-1 ' href="#">
            {e.length === 0 && <span className='bg-zinc-700 inline-block h-full w-[1px]'></span>}
            {i === 1 && <span style={{boxShadow:"0 0 0.25em #00FF19"}} className='bg-[#00FF19] inline-block h-1 w-1 rounded-full'></span>}{e}</a>
        ))}
      </div>
      <Button />
    </div>
